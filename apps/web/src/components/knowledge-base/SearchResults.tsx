...
const SearchResults: React.FC<SearchResultsProps> = () => {
  ...
  const performSearch = async (searchQuery: string) => {
    try {
      setLoading(true);
      const searchResults = await knowledgeBaseService.searchArticles({ query: searchQuery });
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading ? (
          ...
          <div className="space-y-4">
            <div className="text-sm text-gray-500">Found {totalResults} results</div>
            {results.map((article) => (
              <a
                key={article.id}
              ...
              >
                ...
                <p className="mt-1 text-gray-600 line-clamp-2">{article.excerpt}</p>
          ...
          <div className="text-center py-12">
            ...
            <div className="flex items-center justify-center gap-4">
              <a href="/knowledge-base" className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                Back to Knowledge Base
              </a>
              <a href="/knowledge-base/search" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                Clear Search
              </a>
            </div>
          </div>
