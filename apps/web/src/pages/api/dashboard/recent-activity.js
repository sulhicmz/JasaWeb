...
export async function GET({ request, url }) {
  try {
    ...
    const backendUrl = buildUrl('/dashboard/recent-activity', { limit });
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        lastError = error;
        console.warn(
          `Recent activity API attempt ${attempt} failed:`,
          error.message
        );
    ...
    console.error('Recent activity API error after retries:', error);
