...
export async function GET({ request, url }) {
  try {
    ...
    const backendUrl = buildUrl('/dashboard/projects-overview', { limit });
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        lastError = error;
        console.warn(
          `Projects overview API attempt ${attempt} failed:`,
          error.message
        );
    ...
    console.error('Projects overview API error after retries:', error);
