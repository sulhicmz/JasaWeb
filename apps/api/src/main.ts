...
async function bootstrap() {
  ...
  app.use(
    helmet({
      contentSecurityPolicy: envService.isProduction
        ? {
            directives: {
              defaultSrc: ["'self'"],
  ...
  // Configuration services are available through global modules

  // Enhanced CORS configuration using dynamic values
  app.enableCors({
    origin: envService.corsOrigins,
  ...
  logger.log(`Application is running on: ${envService.apiUrl}:${port}`);
}
bootstrap();
