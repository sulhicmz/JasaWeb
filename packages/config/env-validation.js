...
exports.EnvValidationError = exports.ENV_SCHEMA = void 0;
...
exports.ENV_SCHEMA = {
    ...
    JWT_SECRET: {
        ...
        minLength: 32,
        pattern: /^[A-Za-z0-9+/=_-]+$/,
        description: 'JWT signing secret (minimum 32 characters, alphanumeric and symbols only)',
    },
    JWT_REFRESH_SECRET: {
        type: 'string',
        required: true,
        minLength: 32,
        pattern: /^[A-Za-z0-9+/=_-]+$/,
        description: 'JWT refresh secret (minimum 32 characters, alphanumeric and symbols only)',
    },
    JWT_EXPIRES_IN: {
        type: 'string',
        required: false,
    ...
    SESSION_SECRET: {
        ...
        minLength: 32,
        pattern: /^[A-Za-z0-9+/=_-]+$/,
        description: 'Session signing secret (minimum 32 characters, alphanumeric and symbols only)',
    },
...
function validateEnvironmentVariables() {
    const errors = [];
    const warnings = [];
    const allowedKeys = new Set(Object.keys(exports.ENV_SCHEMA));
    const envKeys = Object.keys(exports.ENV_SCHEMA);
    for (const key of envKeys) {
        if (!allowedKeys.has(key)) {
            errors.push(`Invalid environment variable key: ${key}`);
            continue;
        }
        const schema = exports.ENV_SCHEMA[key];
        ...
        if (value !== undefined && value !== '') {
            ...
            if (schema?.type === 'string') {
                if (schema?.minLength && value.length < schema.minLength) {
                    errors.push(`Environment variable ${key} must be at least ${schema.minLength} characters long (current: ${value.length})`);
                    continue;
                }
    ...
    if (process.env.NODE_ENV === 'production' ||
        ...
        if ((process.env.POSTGRES_DB === 'test' ||
            process.env.POSTGRES_DB === 'prod' ||
            process.env.POSTGRES_DB === 'production') &&
            process.env.NODE_ENV === 'development') {
            warnings.push(`Using potentially database name in development environment: ${process.env.POSTGRES_DB}`);
        }
        const checkWeakPassword = (value, name) => {
            if (value) {
    ...
    if (warnings.length > 0) {
        logger_1.logger.warn('Environment security warnings', { warnings });
    }
    if (errors.length > 0) {
        logger_1.logger.error('Environment validation errors', { errors });
...
function getRequiredEnv(key) {
    ...
    if (!Object.prototype.hasOwnProperty.call(exports.ENV_SCHEMA, key)) {
        throw new EnvValidationError(`Environment variable ${key} is not in schema`);
    }
    const value = process.env[key];
    if (!value) {
